...
void print_buffer(char output_buffer[], int *buff_ind);
...
/**
 * print_buffer - Prints the contents of the output_buffer if it exist
...
{
	if (*buff_ind > 0)
		write(1, &output_buffer[0], *buff_ind);
	...
	int flags, int width, int precision, int size)
{
	...
	return (write_number(is_negative, i, output_buffer,
				flags, width, precision, size));
}
